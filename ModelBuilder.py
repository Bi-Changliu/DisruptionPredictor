...
def CNNForStoredEnergy(StoredEnergy, DropoutProb, IsTraining):
...
def CNNForMir_Pol_A(Mir_Pol_A, DropoutProb, IsTraining):
    with tf.name_scope('CNNForMir_Pol_A'):
        Param=SettingsDict['ModelParamDict']['CNNForMir_Pol_A']
        InputShape=Mir_Pol_A.get_shape()
        TimeStepSize=InputShape[1]
        """============================reshape输入=============================="""
        InputLength = SettingsDict['InputParamDict']['SignalDict']['Mir_Pol_A']['FreqDoub']
        FingerprintInput = tf.reshape(Mir_Pol_A,[-1, InputLength, 1])
...
def RNNForAllSignals(AllSignals, DropoutProb):
    with tf.name_scope('RNNForAllSignals'):
        ...
        AllSignalsShape = AllSignals.get_shape()
        BatchSplitSize  = AllSignalsShape[0]
        ...
        MultiLSTMCell = tf.contrib.rnn.MultiRNNCell([LSTMCell] * LayerNum, 
                                                            state_is_tuple=True)
        InitState = MultiLSTMCell.zero_state(BatchSplitSize, dtype=tf.float32)
        ...
        FC1Weights = tf.Variable(tf.truncated_normal(
                [HiddenSize, FC1CellNum], stddev=0.1))
        FC1Bias = tf.Variable(tf.zeros([FC1CellNum]))
        ...
        FC2Weights = tf.Variable(tf.truncated_normal(
                [FC1CellNum, FC2CellNum], stddev=0.1))
        FC2Bias = tf.Variable(tf.zeros([FC2CellNum]))
        FC2Output = tf.matmul(FC1Output, FC2Weights) + FC2Bias
